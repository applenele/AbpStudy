...
namespace AbpStudy.Tests
{
    public abstract class AbpStudyTestBase : AbpIntegratedTestBase<AbpStudyTestModule>
    {
        ...
        protected Task UsingDbContextAsync(Func<AbpStudyDbContext, Task> action)
        {
        ...
        protected Task<T> UsingDbContextAsync<T>(Func<AbpStudyDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpStudyDbContext> action)
        {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpStudyDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpStudyDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpStudyDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpStudyDbContext, Task<T>> func)
        {
            T result;
